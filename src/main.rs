...
use model::{KeyValue, ServerConfig, ServerInfo};
...
fn parse_log_level_from_env() -> Option<LogLevel> {
    get_env_with_fallback("TREDIS_LOG_LEVEL", "LOG_LEVEL").and_then(|s| {
...
pub struct Args {
    /// Redis host [env: TREDIS_HOST, HOST]
    #[arg(short = 'H', long, default_value_t = get_env_with_fallback("TREDIS_HOST", "HOST").unwrap_or_else(|| "localhost".to_string()))]
    pub host: String,

    /// Redis port [env: TREDIS_PORT, PORT]
    #[arg(short, long, default_value_t = get_env_with_fallback("TREDIS_PORT", "PORT").and_then(|s| s.parse().ok()).unwrap_or(6379))]
    pub port: u16,

    /// Redis database [env: TREDIS_DB, DB]
    #[arg(short, long, default_value_t = get_env_with_fallback("TREDIS_DB", "DB").and_then(|s| s.parse().ok()).unwrap_or(0))]
    pub db: i64,

    /// Log level (off, error, warn, info, debug) [env: TREDIS_LOG_LEVEL, LOG_LEVEL]
    #[arg(short, long, default_value_t = parse_log_level_from_env().unwrap_or(LogLevel::Off), value_enum)]
...
async fn main() -> Result<()> {
    ...
    if has_cli_override {
        ...
        app.current_server = Some(ServerConfig {
            name: format!(
                "{}:{}",
                app.connection_config.host, app.connection_config.port
            ),
            uri: format!(
                "redis://{}:{}/{}",
                app.connection_config.host, app.connection_config.port, app.connection_config.db
            ),
            info: None,
        });
        ...
        tokio::spawn(async move {
            ...
            let _ = tx_clone
                .send(AppEvent::Progress("Connecting to Redis...".to_string()))
                .await;
                        ...
                        if app.info_search_active {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if app.pubsub_subscribe_channel.is_empty()
                                        && !app.pubsub_subscribe_input.is_empty()
                                    {
                                        // Start subscription
                                        ...
                                        let uri = if let Some(ref server) = app.current_server {
                                            ...
                                            let scheme = if app.connection_config.tls {
                                                "rediss"
                                            } else {
                                                "redis"
                                            };
                                        ...
                                        let task = tokio::spawn(async move {
                                            if let Ok(client) = redis::Client::open(uri) {
                                                    ...
                                                    client.get_async_pubsub().await
                                                {
                                                    ...
                                                    {
                                                        let payload: String =
                                                            msg.get_payload().unwrap_or_default();
                                                        ...
                                                        let _ = tx_clone
                                                            .send(AppEvent::PubSubMessage(entry))
                                                            .await;
                                                    }
                                                }
                                            }
                            ...
                            match key.code {
                                KeyCode::Esc => {
                                    ...
                                    if app.active_resource == "info"
                                        && !app.info_search_text.is_empty()
                                    {
                                    ...
                                    match app.active_resource.as_str() {
                                        "servers" => {
                                            if !app.tredis_config.servers.is_empty()
                                                && app.selected_server_index
                                                    < app.tredis_config.servers.len() - 1
                                            {
                                        ...
                                        "clients" => {
                                            if !app.clients.is_empty()
                                                && app.selected_client_index < app.clients.len() - 1
                                            {
                                        ...
                                        "slowlog" => {
                                            if !app.slowlogs.is_empty()
                                                && app.selected_slowlog_index
                                                    < app.slowlogs.len() - 1
                                            {
                                        ...
                                        "config" => {
                                            if !app.configs.is_empty()
                                                && app.selected_config_index < app.configs.len() - 1
                                            {
                                        ...
                                        "acl" => {
                                            if !app.acls.is_empty()
                                                && app.selected_acl_index < app.acls.len() - 1
                                            {
                                        ...
                                        "monitor" => {
                                            if !app.monitor_entries.is_empty()
                                                && app.selected_monitor_index
                                                    < app.monitor_entries.len() - 1
                                            {
                                                app.selected_monitor_index += 1;
                                                // Only scroll if needed (selected item goes out of view)
                                                if app.selected_monitor_index
                                                    >= app.monitor_scroll + 10
                                                {
                                                    app.monitor_scroll =
                                                        app.monitor_scroll.saturating_add(1);
                                                }
                                            }
                                        }
                                        "streams" => {
                                            if !app.streams.is_empty()
                                                && app.selected_stream_index < app.streams.len() - 1
                                            {
                                        ...
                                        "pubsub" => {
                                            if !app.pubsub_channels.is_empty()
                                                && app.selected_pubsub_index
                                                    < app.pubsub_channels.len() - 1
                                            {
                                        ...
                                        "monitor" => {
                                            if app.selected_monitor_index > 0 {
                                                ...
                                                if app.selected_monitor_index < app.monitor_scroll {
                                                    app.monitor_scroll =
                                                        app.monitor_scroll.saturating_sub(1);
                                                }
                                            }
                                        }
                                        "streams" => {
                                ...
                                KeyCode::Char('R') => {
                                    match app.active_resource.as_str() {
                                        "clients" => {
                                            let _ = app.fetch_clients().await;
                                        }
                                        "info" => {
                                            let _ = app.fetch_info().await;
                                        }
                                        "slowlog" => {
                                            let _ = app.fetch_slowlog().await;
                                        }
                                        "config" => {
                                            let _ = app.fetch_configs().await;
                                        }
                                        "acl" => {
                                            let _ = app.fetch_acls().await;
                                        }
                                        "monitor" => {
                                            /* Monitor is real-time, cleared on refresh */
                                        ...
                                        "pubsub" => {
                                            let _ = app.fetch_pubsub_channels().await;
                                        }
                                        _ => {
                                ...
                                KeyCode::Char('c') => {
                                    ...
                                    if app.active_resource == "servers"
                                        && !app.tredis_config.servers.is_empty()
                                    {
                                        log!(
                                            LogLevel::Info,
                                            "[CONNECT] 'c' pressed in servers view"
                                        );
                                        log!(
                                            LogLevel::Info,
                                            "[CONNECT] Selected server index: {}",
                                            app.selected_server_index
                                        );
                                        log!(
                                            LogLevel::Info,
                                            "[CONNECT] Total servers: {}",
                                            app.tredis_config.servers.len()
                                        );

                                        let server = app.tredis_config.servers
                                            [app.selected_server_index]
                                            .clone();
                                        log!(
                                            LogLevel::Info,
                                            "[CONNECT] Server name: {}, URI: {}",
                                            server.name,
                                            mask_uri(&server.uri)
                                        );
                                        ...
                                        if let Err(e) = app.set_connection_from_uri(&server.uri) {
                                            ...
                                            log!(
                                                LogLevel::Info,
                                                "[CONNECT] URI parsed successfully"
                                            );
                                            log!(
                                                LogLevel::Info,
                                            ...
                                            if let Some(conn) = app.connection.take() {
                                                ...
                                                log!(
                                                    LogLevel::Info,
                                                    "[CONNECT] Dropped existing connection"
                                                );
                                            }
                                            if let Some(client) = app.client.take() {
                                                ...
                                                log!(
                                                    LogLevel::Info,
                                                    "[CONNECT] Dropped existing client"
                                                );
                                            }
                                            ...
                                            log!(
                                                LogLevel::Info,
                                                "[CONNECT] Reset client and connection"
                                            );

                                            app.mode = Mode::Splash;
                                            app.splash_state =
                                                crate::ui::splash::SplashState::new();
                                            ...
                                            tokio::spawn(async move {
                                                ...
                                                if needs_detection {
                                                    let _ = tx_clone
                                                        .send(AppEvent::Progress(
                                                            "Detecting server type...".to_string(),
                                                        ))
                                                        .await;
                                                    if let Ok(info) =
                                                        App::detect_server_info(&uri_clone).await
                                                    {
                                                        let _ = tx_clone
                                                            .send(AppEvent::ServerInfoDetected {
                                                ...
                                                log!(
                                                    LogLevel::Info,
                                                    "[CONNECT-TASK] Sending Progress event"
                                                );
                                                let _ = tx_clone
                                                    .send(AppEvent::Progress(
                                                        "Connecting to Redis...".to_string(),
                                                    ))
                                                    .await;
                                                log!(
                                                    LogLevel::Info,
                                                    "[CONNECT-TASK] Sending Connect event"
                                                );
                                                let _ = tx_clone.send(AppEvent::Connect).await;
                                                log!(
                                                    LogLevel::Info,
                                    ...
                                    else if app.active_resource == "streams"
                                        && !app.streams.is_empty()
                                    {
                                        ...
                                        log!(
                                            LogLevel::Debug,
                                            "[MAIN] Spawning consumer task for stream: {}",
                                            stream_name
                                        );
                                        let task = tokio::spawn(async move {
                                            log!(
                                            ...
                                            if let Ok(client) = redis::Client::open(format!(
                                                ...
                                                log!(
                                                    LogLevel::Debug,
                                                    "[TASK] Client created, getting connection..."
                                                );
                                                if let Ok(mut con) =
                                    ...
                                    if key.modifiers.contains(KeyModifiers::CONTROL) =>
                                {
                                    ...
                                    {
                                        let server =
                                            &app.tredis_config.servers[app.selected_server_index];
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if app.active_resource == "servers" {
                                        app.server_dialog_state =
                                            crate::ui::server_dialog::ServerDialogState::new();
                                        app.mode = Mode::ServerDialog;
                                    }
                                }
                                KeyCode::Char('d') => {
                                        ...
                                        && !app.tredis_config.servers.is_empty()
                                    {
                                        let server =
                                            &app.tredis_config.servers[app.selected_server_index];
                                        app.describe_data =
                                            KeyValue::String(format_server_details(server));
                        ...
                        match key.code {
                            ...
                            KeyCode::Right | KeyCode::Tab => {
                                if !app.command_suggestions.is_empty() {
                                    if let Some(selected) =
                                        app.command_suggestions.get(app.command_suggestion_selected)
                                    {
                            ...
                            KeyCode::Enter => {
                                if let Some(selected) = app
                                    .command_suggestions
                                    .get(app.command_suggestion_selected)
                                    .cloned()
                                {
                                    // Stop monitor/pubsub/stream consumers if switching away from them
                                    if app.active_resource == "monitor"
                                        && selected.command != "monitor"
                                    {
                                        app.stop_monitor();
                                    }
                                    if app.active_resource == "pubsub"
                                        && selected.command != "pubsub"
                                    {
                                    ...
                                    if app.active_resource == "streams"
                                        ...
                                        "monitor" => {
                                            ...
                                            let task = tokio::spawn(async move {
                                                ...
                                                if let Ok(stream) = TcpStream::connect(&addr).await
                                                {
                                                    ...
                                                    let monitor_cmd = "*1\r\n$7\r\nMONITOR\r\n";
                                                    if writer
                                                        .write_all(monitor_cmd.as_bytes())
                                                        .await
                                                        .is_ok()
                                                    {
                            ...
                            KeyCode::Enter => {
                                if app.server_dialog_state.is_valid() {
                                    ...
                                    if let Err(e) = app.add_server_from_dialog() {
                                        ...
                                        let uri =
                                            app.current_server.as_ref().map(|s| s.uri.clone());
                                        let server_name =
                                            app.current_server.as_ref().map(|s| s.name.clone());
                                        if let (Some(uri), Some(name)) = (uri, server_name) {
                                            if let Err(e) = app.set_connection_from_uri(&uri) {
                                                app.server_dialog_state
                                                    .set_error(format!("Invalid URI: {}", e));
                                            } else {
                                                // Success! Switch to splash and connect
                                                app.mode = Mode::Splash;
                                                app.splash_state
                                                    .set_message("Detecting server type...");
                                    ...
                                    app.server_dialog_state
                                        .set_error("Please fill in all fields".to_string());
                                }
                            }
                            _ => {}
                        }
...
fn format_server_details(server: &model::ServerConfig) -> String {
    ...
    if let Some(ref info) = server.info {
        ...
        server_info.insert(
            "type".to_string(),
            serde_json::Value::String(info.server_type.as_str().to_string()),
        );
        server_info.insert(
            "version".to_string(),
            serde_json::Value::String(info.redis_version.clone()),
        );
        server_info.insert(
            "role".to_string(),
            serde_json::Value::String(info.role.clone()),
        );
        if !info.os.is_empty() {
        ...
        if let Some(cluster_size) = info.cluster_size {
            server_info.insert(
                ...
                serde_json::Value::Number(serde_json::Number::from(cluster_size)),
            );
        }
        json_obj.insert(
    ...
    connection.insert(
        "uri".to_string(),
        serde_json::Value::String(mask_uri(&server.uri)),
    );
